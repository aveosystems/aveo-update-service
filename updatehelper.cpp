...
BOOL PathGetSiblingFilePath(LPWSTR destinationBuffer, LPCWSTR siblingFilePath,
  ...
  wcsncpy_s(destinationBuffer, MAX_PATH + 1, siblingFilePath, MAX_PATH);
 ...
 * launched using the update service.
 *
 * Example
 * Destination buffer value:
 *   C:\Program Files\Aveo Systems\Update Service\UpdateLogs
 *
 * @param  outBuf
...
BOOL GetSecureOutputDirectoryPath(LPWSTR outBuf) {
  ...
  wcsncpy_s(outBuf, MAX_PATH + 1, progFilesX86, MAX_PATH);
  CoTaskMemFree(progFilesX86);

  if (!PathAppendSafe(outBuf, L"Aveo System\\Update Service")) {
      return FALSE;
  }

  // Create the Update Service directory in case it doesn't exist.
 ...
 *   C:\Windows\ServiceProfiles\LocalService\AppData\Local\Aveo Systems\Mira Connect\
 *       updates\0123456789ABCDEF\updates\0
...
BOOL GetSecureOutputFileName(LPCWSTR patchDirPath, LPCWSTR fileExt,
  ...
  // an elevated updater or an updater that is launched using the update
  // service.
  ...
  wcsncpy_s(outBuf, MAX_PATH + 1, patchDirPath, MAX_PATH);
  ...
  wcsncat_s(outBuf, MAX_PATH + 1, fileExt, fileExtLen);
 ...
 *   .status
 * Destination buffer value:
 *   C:\Program Files (x86)\Aveo Systems\Mira Connect\UpdateLogs\0123456789ABCDEF.status
 *
 * @param  patchDirPath
 *         The path to the update patch directory.
 * @param  fileExt
...
BOOL WriteSecureIDFile(LPCWSTR patchDirPath) {
  ...
  FILE* idFile;
  errno_t error = _wfopen_s(&idFile, idFilePath, L"wb+");
  if (error != 0 || idFile == nullptr) {
    return FALSE;
  }
...
BOOL StartServiceUpdate() {
  ...
  // Obtain the temp path of the update service binary
  WCHAR tmpService[MAX_PATH + 1] = { L'\0' };
  if (!PathGetSiblingFilePath(tmpService, serviceConfig.lpBinaryPathName,
                              L"updateservice_tmp.exe")) {
    return FALSE;
  }
  LOG(("temp service path = %ls", tmpService));

  WCHAR tmpServicePath[MAX_PATH + 1] = { L'\0' };
  wcsncpy_s(tmpServicePath, MAX_PATH + 1, tmpService, MAX_PATH);
  if (!PathRemoveFileSpecW(tmpServicePath)) {
      return FALSE;
  }

  // Get the new update service path from the install dir
  WCHAR newUpdateServicePath[MAX_PATH + 1] = {L'\0'};
  wcsncpy_s(newUpdateServicePath, MAX_PATH + 1, tmpServicePath, MAX_PATH);
  PathAppendSafe(newUpdateServicePath, L"updateservice.exe");
  LOG(("new service path = %ls", newUpdateServicePath));

  // The installer for the update service will have already copied the
  // temporary version of updateservice_tmp.exe alongside the running
  // version of the update service. This is a requirement for update service 
  // upgrades.
  ...
  if (!DoesBinaryMatchAllowedCertificates(tmpServicePath, tmpService)) {
  ...
  WCHAR cmdLine[64] = {'\0'};
  wcsncpy_s(cmdLine, sizeof(cmdLine) / sizeof(cmdLine[0]), L"dummyparam.exe upgrade",
...
/**
 * Executes a update service command
 *
...
LaunchServiceSoftwareUpdateCommand(int argc, LPCWSTR* argv) {
  ...
  // 1) The command being executed, which is "software-update"
  // 2) The path to updater.exe (from argv[0])
  LPCWSTR* updaterServiceArgv = new LPCWSTR[argc + 1];
  updaterServiceArgv[0] = L"software-update";

  for (int i = 0; i < argc; ++i) {
    updaterServiceArgv[i + 1] = argv[i];
  }

  // Execute the service command by starting the service with
  // the passed in arguments.
  DWORD ret = StartServiceCommand(argc + 1, updaterServiceArgv);
...
BOOL WriteStatusFailure(LPCWSTR patchDirPath, int errorCode) {
  ...
  char failure[32];
  sprintf_s(failure, 32, "failed: %d", errorCode);
  DWORD toWrite = static_cast<DWORD>(strlen(failure));
  DWORD wrote;
...
IsProcessRunning(LPCWSTR filename) {
  ...
  do {
    if (_wcsicmp(filename, processEntry.szExeFile) == 0) {
      CloseHandle(snapshot);
...
BOOL IsLocalFile(LPCWSTR file, BOOL& isLocal) {
  ...
  wcsncpy_s(rootPath, MAX_PATH + 1, file, MAX_PATH);
