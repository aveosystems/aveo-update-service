...
DIR::DIR(const WCHAR* path) : findHandle(INVALID_HANDLE_VALUE) {
  ...
  wcsncpy_s(name, sizeof(name) / sizeof(name[0]), path, sizeof(name) / sizeof(name[0]) - 1);
  wcsncat_s(name, sizeof(name) / sizeof(name[0]), L"\\*", sizeof(name) / sizeof(name[0]) - wcslen(name) - 1);
}
...
dirent* readdir(DIR* dir) {
  ...
  wcsncpy_s(gDirEnt.d_name, direntBufferLength, data.cFileName, direntBufferLength - 1);
...
BOOL GetUUIDString(LPWSTR outBuf) {
  ...
  wcsncpy_s(outBuf, MAX_PATH + 1, reinterpret_cast<LPCWSTR>(uuidString), MAX_PATH);
...
BOOL GetUUIDTempFilePath(LPCWSTR basePath, LPCWSTR prefix, LPWSTR tmpPath) {
  ...
  if (prefix) {
    ...
    wcsncpy_s(filename, MAX_PATH + 1, prefix, MAX_PATH);
  }
  ...
  wcsncat_s(filename, MAX_PATH + 1, tmpFileNameString, tmpFileNameStringLen);
  ...
  wcsncpy_s(tmpPath, basePathLen + 1, basePath, basePathLen);
